...
| GHSA-mwcw-c2x4-8c55 (+Excalidraw chain advisories via npm audit) | @excalidraw/excalidraw | Moderate | yes | 0.17.1-08b13f9 - 0.17.1-test \|\| >=0.18.0-6135548 | yes (`@excalidraw/excalidraw@0.17.6`, semver-major downgrade) | fix-next-sprint | MUWI maintainers | fix-now (completed 2026-02-25 via patched `@excalidraw/mermaid-to-excalidraw` git fork pin) | 2026-02-25 | Full downgrade is blocked by React peer mismatch (`0.17.6` requires React `^17 || ^18`). Added targeted overrides to reduce Excalidraw transitive risk (`@excalidraw/excalidraw -> nanoid@3.3.11`, `@excalidraw/mermaid-to-excalidraw -> mermaid@10.9.5`). Follow-up experiment on 2026-02-23 tested nested npm override `@excalidraw/mermaid-to-excalidraw -> nanoid@3.3.11`, but npm did not converge the installed tree (nested `nanoid@4.0.2` remained) and `npm ls` reported `ELSPROBLEMS` invalid dependency state, so the override was reverted and recorded as a no-go. Final remediation (2026-02-25): replaced transitive `@excalidraw/mermaid-to-excalidraw` with a commit-pinned patched GitHub fork that bumps `mermaid` to `10.9.5` and nested `nanoid` to `5.0.9`; `npm audit` no longer reports the Excalidraw chain. `muwi/package.json` uses `git+https://`, while npm `11.4.2` canonicalized `package-lock.json` to `git+ssh://git@github.com/...`; installs without GitHub SSH may require Git URL rewrite to HTTPS or an internal package/tarball publication. | `muwi/audit/2026-02/outputs/day8-audit-deps-before-excalidraw-fork-json.txt`; `muwi/audit/2026-02/outputs/day8-audit-deps-before-excalidraw-fork-json.txt.summary.md`; `muwi/audit/2026-02/outputs/day8-audit-deps-after-excalidraw-fork-json.txt`; `muwi/audit/2026-02/outputs/day8-audit-deps-after-excalidraw-fork-json.txt.summary.md`; `muwi/audit/2026-02/outputs/day8-npm-ls-excalidraw-chain-before.txt`; `muwi/audit/2026-02/outputs/day8-npm-ls-excalidraw-chain-after.txt`; `npm install`; `npm run lint`; `npm run test -- --run src/components/diaries/blackboard/ExcalidrawWrapper.test.tsx src/components/diaries/blackboard/Blackboard.test.tsx`; `npm run build` |
...
| 2026-02-25 | Excalidraw dependency-level remediation via patched `@excalidraw/mermaid-to-excalidraw` git-fork pin | Success: `npm install` converged to `@excalidraw/mermaid-to-excalidraw@1.1.2` (commit-pinned GitHub fork) with nested `nanoid@5.0.9`; `npm audit` reduced from `1 high + 3 moderate` to `1 high` (`0 moderate`), clearing the Excalidraw runtime-path chain. `muwi/package.json` uses `git+https://`, but npm lockfile canonicalized to `git+ssh://git@github.com/...`. | Keep upstream watch for official fix. If CI/runtime environments do not have GitHub SSH access, configure Git URL rewrite to HTTPS or switch this dependency pin to an internal package/tarball publication. |
| | | | |
